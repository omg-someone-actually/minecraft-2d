...
class Minecraft:
    ...
    def screen_update(self) -> None:
        selectable_blocks = ['stone', 'coal', 'gold','redstone', 'grass','sand', 'water', 'dirt', 'glass', 'plank']
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        ...
                        if not pygame.sprite.collide_rect(new_block, self.player):
                    ...
                    if event.key == pygame.K_r:
                        ...
                        self.selected_block = selectable_blocks[event.key-48]
            ...
            moves = {pygame.K_UP: 'jump', pygame.K_LEFT: 'left', pygame.K_RIGHT: 'right'}
            new_move = None
            ...
            self.x, self.y = self.player.move(new_move, self.blocks[self.x][self.y], self.x, self.y)
            self.generate_map()
            for zombie in self.zombies:
                zombie.move(self.blocks[self.x][self.y], self.player)
